...
export async function ejeFormativosDeleteById(
	req: HttpRequest,
	...
	try {
		...
		const isDeleted = await ejeFormativoService.deleteEjeFormativoById(id);

		if (!isDeleted) {
			return {
				jsonBody: {
					message: "No se ha eliminado la ejeFormativo.",
				},
				status: 400,
			};
		}
		...
		return {
			jsonBody: { message: "EjeFormativo eliminado con exito." },
			status: 200,
		};
	} catch (error: any) {
		ctx.error(error);
...
app.http("ejeFormativosDeleteById", {
	methods: ["DELETE"],
	authLevel: "anonymous",
	handler: ejeFormativosDeleteById,
